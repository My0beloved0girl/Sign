    ...
    $("#doSubmit").click(function(){
    	var like = $("#likename").val()
    	 tableIns.reload({
	      ...
	      url:"http://localhost:8080/Sign/ZHS?action=sszh&uname="+like,
	      page: {
	        curr: 1 //重新从第 1 页开始
	      }
	    });
    		...
    		success:function(layero, index){
    			$.ajax({
    				...
    				success:function(data){	
    					...
    					body.find("#uname2").val(info.data[0].loginName);
    					body.find("#uname").val(info.data[0].loginName);
    					body.find("#password").val(info.data[0].password);
    					body.find("#realName").val(info.data[0].userName);
    					body.find("#email").val(info.data[0].email);
    					body.find("#phone").val(info.data[0].mobile);
    					body.find("#banji").val(info.data[0].clas);
    					body.find("#sfz").val(info.data[0].identityCode);
    					//性别(单选)
