layui.use(['form','layer','layedit','laydate','upload'],function(){
    ...
    $("#mname").blur(function(){
    	...
    	$.post("../QXS?action=qxname&mname="+mname,function(res){
    		/*var cs = eval('(' + res + ')');*/
    		if(res == 1){
    			layer.msg('权限名已存在');
    		}
...
*/
    ...
    form.on('submit(menuUser)',function(data){
    	...
    	if(type2==1){
            ...
            $.post("../QXS?action=insqx&mname="+mname+"&type2="+type2+"&father="+father,function(data){
    	 		if(data == 1){
    	  ...
    	  //请求路径
            ...
            if(type2 != 1){
            	if(father == 0){
            		layer.msg("目录或按钮不能没有上级");
                	return false;
            	}
            }
            $.post("../QXS?action=insqx&mname="+mname+"&mfunction="+mfunction+"&type2="+type2+"&mbtn="+mbtn+"&father="+father,function(data){
    		...
    		if(type2 == 3){
    		 ...
    		 $.post("../QXS?action=insqx&mname="+mname+"&type2="+type2+"&mbtn="+mbtn+"&father="+father,function(data){
     	 		if(data == 1){
     	 			top.layer.msg("权限添加成功");
     	 			layer.closeAll("iframe");
     	            //刷新父页面
    ...
    form.on('select(type2)', function(data){
        ...
        if(type == 3){
        	$("#mbtn").val('');
        	$("#lujin").css("display","none");
        	$("#btnclss").css("display","block");
        	$("#zhixingti").css("display","block");
        	$.get("../QXS?action=sean",function (res) {
        	...
        	$("#lujin").css("display","none");
        	$("#zhixingti").css("display","none");
        	$("#btnclss").css("display","none");
        }else{
        	$("#zhixingti").css("display","block");
        	$("#lujin").css("display","block");
        	$("#btnclss").css("display","none");
        }
        $.get("../QXS?action=qxpd&type="+type,function (res) {
        	/*console.log(res);*/
        	var cs = eval('(' + res + ')');
         /*   var data = cs.data;*/
            var dom = $("#father");
